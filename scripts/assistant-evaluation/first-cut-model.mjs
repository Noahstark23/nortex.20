...
export async function runFirstCutModelEvaluation(args = process.argv.slice(2), deps = {}) {
  ...
  const initial = { version: 1, status: 'not_run', paidCalls: 0, humanReview: 'pending',
  ...
  return withEvaluationReport(output, resume, async (existing, save) => {
    if (existing && (existing.version !== 1 || existing.binding !== binding || !existing.conversationId || !existing.requestId
