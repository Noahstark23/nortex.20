...
const itemSchema = z.object({
  ...
  productId: text.optional(), productName: text.optional(), productBrand: text.nullable().optional(), baseUnit: text.optional(),
...
const candidateSchema = z.object({
  id: text, label: text, detail: z.string().max(3000).optional(), sku: text.optional(), brand: text.nullable().optional(),
  unit: text.optional(), packUnit: text.nullable().optional(), packSize: text.nullable().optional(),
  saleMode: text.nullable().optional(), quantityStep: text.nullable().optional(), requiresBatchTracking: z.boolean().optional(),
  ruc: text.nullable().optional(), address: z.string().max(2000).nullable().optional(), selectionIssue: text.optional(),
}).strict();
export const purchaseIntakeSchema = z.object({
  ...
  facts: z.object({
    items: z.array(itemSchema).min(1).max(200), supplierName: text.optional(), supplierCatalogName: text.optional(), supplierId: text.optional(),
  ...
  pendingQuestion: z.object({ id: z.string().uuid(), field: z.string().max(64), searchQuery: text.optional(), lineIndex: z.number().int().min(0).optional(),
    candidates: z.array(candidateSchema).max(6).optional(),
  }).strict().optional(),
