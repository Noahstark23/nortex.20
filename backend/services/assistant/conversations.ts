...
import { assistantMessageUsageKey, createAssistantLanguage, type AssistantReadPlan } from './language.js';
...
export async function sendAssistantMessage(
    ...
    dependencies:{channel?:AssistantKnowledgeChannel;startRun?:(principal:AssistantPrincipal,id:string,db:PrismaClient)=>Promise<unknown>;
      interpret?:(principal:AssistantPrincipal,text:string,history:string[],usageKey:string)=>Promise<AssistantReadPlan|null>}={},
    ...
    const plan=useOperational||fixedIntent==='restricted'?null:await (dependencies.interpret??createAssistantLanguage({db}))(principal,text,history,assistantMessageUsageKey(conversationId,requestId));
