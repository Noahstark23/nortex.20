...
import { retrievePublishedAssistantHelp } from '../knowledge/service.js';
...
interface ToolDependencies {channel?:AssistantKnowledgeChannel;db?:PrismaClient;now?:()=>Date;extraTools?:OperationTool[]}
...
export async function createOperationTools(principal:AssistantPrincipal,deps:ToolDependencies={}):Promise<OperationTool[]> {
  ...
  const registry:OperationTool[]=[{name:'search_help',label:'Ayuda de Nortex',description:'Recuperar ayuda oficial de Nortex con referencias exactas; incluye ayuda heredada y publicaciones revisadas. No consultar datos operativos aquí.',kind:'READ',schema:z.object({query:z.string().trim().min(1).max(500)}).strict(),async execute(ctx,args){await authorize(ctx);const {query}=z.object({query:z.string()}).parse(args);const result=await retrievePublishedAssistantHelp(principal,query,db,ctx.channel??deps.channel??'WEB_INTERNAL');return{data:asJson(result),knowledgeReferences:result.knowledgeReferences};}}];
