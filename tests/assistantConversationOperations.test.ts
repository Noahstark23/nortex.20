...
describe('enrutamiento de operaciones en la misma conversación',()=>{
  it.each(['Compré 50 bolsas de cemento','Completar por aquí','cancelar compra','sí','C$ 230','son 60, no 50'])('conserva respuestas de compra: %s',text=>{
    expect(shouldUseOperationalRun(text,true)).toBe(false);
  });
  it.each(['¿Cómo va mi negocio?','Compará ventas y gastos','Comparalo con ayer','Prepará una orden de compra','Revisá los vencidos','Explicá el aviso a56f','Buscá cemento'])('permite interrumpir compra con consulta: %s',text=>{
