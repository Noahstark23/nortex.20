...
describe('customer hub route safeguards', () => {
    ...
    it('prefiltra inactivos con el mismo dia civil Managua que usa la clasificacion final', () => {
        ...
        expect(server).toContain('const cutoff = new Date(inicioDelDiaManagua(asOf).getTime() - 59 * 86400000);');
        expect(server).toContain('createdAt: { lt: cutoff },');
        expect(server).toContain('createdAt: { gte: cutoff },');
