...
beforeEach(async () => { localStorage.clear(); sessionStorage.clear(); await db.offline_sales.clear(); /* Giro EXPLÍCITO: este caso recorre el cobro entero en el POS SIMPLE (rótulos
   "Cobrar C$ … en efectivo", "Registrar efectivo y seguir"), y el modo simple
   depende del giro (resolvePosSimple). Sin `type` se apoyaba en que el POS
   arrancaba simple para todos — el bug del descuento. La promoción no depende
   del modo; lo que se fija acá es el flujo de cobro tal como se escribió. */
localStorage.setItem('nortex_tenant_data', JSON.stringify({ id: 't1', businessName: 'QA', type: 'PULPERIA' })); localStorage.setItem('nortex_user', JSON.stringify({ id: 'u1', name: 'Caja QA', role: 'CASHIER', tenant: { id: 't1', type: 'PULPERIA' } })); localStorage.setItem('nortex_token', 'synthetic'); localStorage.setItem('token', 'synthetic'); vi.spyOn(navigator, 'onLine', 'get').mockReturnValue(true); vi.stubGlobal('matchMedia', vi.fn((media: string) => ({ matches: media === '(min-width: 1024px)', media, addEventListener: vi.fn(), removeEventListener: vi.fn(), addListener: vi.fn(), removeListener: vi.fn() }))); });
