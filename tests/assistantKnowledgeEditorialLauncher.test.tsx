...
vi.mock('../components/admin/knowledge/KnowledgeEditorialPanel', () => ({ default: function Fixture() {
    const [draft, setDraft] = useState('');
    return <label>Observación pendiente<input value={draft} onChange={event => setDraft(event.target.value)} /></label>;
} }));
afterEach(cleanup);

it('abre bajo demanda y conserva la observación al ocultar y volver a abrir', async () => {
    render(<AssistantKnowledgeEditorial />);
