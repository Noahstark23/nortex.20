...
function harness(role='OWNER'){
  const mocks={assistantKnowledgeControl:{findUnique:vi.fn().mockResolvedValue(null)},assistantKnowledgeVersion:{findMany:vi.fn().mockResolvedValue([])},user:{findFirst:vi.fn().mockResolvedValue({id:'user-a',role,status:'ACTIVE'})},employee:{findFirst:vi.fn().mockResolvedValue(null)},assistantTenantConfig:{findUnique:vi.fn().mockResolvedValue({enabled:true,operationsEnabled:true})}};
