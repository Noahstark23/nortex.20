...
function harness(role='OWNER'){
  const mocks={user:{findFirst:vi.fn().mockResolvedValue({id:'user-a',role,status:'ACTIVE'})},assistantTenantConfig:{findUnique:vi.fn().mockResolvedValue({enabled:true,operationsEnabled:true})}};
