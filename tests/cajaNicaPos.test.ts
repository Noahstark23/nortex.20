...
describe('cobro sin atajos peligrosos', () => {
    ...
    it('la caja guiada cobra efectivo directo y mantiene recibido/vuelto dentro del ticket', () => {
        ...
        expect(checkoutDock).toContain('disabled={!ready || processing}');
    });
});

describe('búsqueda operable con Enter', () => {
    it('prioriza SKU exacto y luego agrega el primer resultado visible', () => {
        const searchKeyDown = between(pos, 'const handleSearchKeyDown', 'const resetReturnFlow');
        const exactSkuAt = searchKeyDown.indexOf('const exactSku');
        const firstVisibleAt = searchKeyDown.indexOf('const firstVisible');
        const selectionAt = searchKeyDown.indexOf('const selected = exactSku ?? firstVisible');

        expect(exactSkuAt).toBeGreaterThanOrEqual(0);
        expect(firstVisibleAt).toBeGreaterThan(exactSkuAt);
        expect(selectionAt).toBeGreaterThan(firstVisibleAt);
        expect(searchKeyDown).toContain('buscarProductos(indiceProductos, term, 1).visibles[0]');
    ...
    it('conserva la búsqueda y anuncia el error cuando no hay coincidencias', () => {
        const searchKeyDown = between(pos, 'const handleSearchKeyDown', 'const resetReturnFlow');
        const noResult = between(searchKeyDown, 'if (!selected)', 'agregarDesdeGrilla(selected)');
...
describe('catálogo táctil y accesible', () => {
    ...
    it('mantiene estructura de lista y no activa productos agotados', () => {
        ...
        expect(catalog).toContain('if (!blocked) onAdd(product)');
