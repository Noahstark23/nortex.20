...
const AccountsReceivable: React.FC = () => {
  ...
  const loadStatementByCustomerId = useCallback(async (customerId: string, seed?: Partial<WorklistItem>) => {
    ...
    setDetailError('');
    try {
      const res = await authFetch(`/api/customers/${encodeURIComponent(customerId)}/statement`, { headers });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
  ...
  const handleWriteoff = async (event?: React.FormEvent) => {
    ...
    try {
      const res = await authFetch(`/api/credits/${encodeURIComponent(writeoffDraft.saleId)}/writeoff`, {
        method: 'POST',
        headers,
