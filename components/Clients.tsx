                ...
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
                    ...
                    <form onSubmit={handleCreate} className="p-6 space-y-4">
                        <div>
                            ...
                            <input required className="w-full border border-slate-300 rounded p-2 mt-1" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-mono font-bold text-slate-500">DNI / RUC</label>
                                <input className="w-full border border-slate-300 rounded p-2 mt-1" value={formData.taxId} onChange={e => setFormData({...formData, taxId: e.target.value})} />
                            </div>
                            <div>
                                <label className="text-xs font-mono font-bold text-slate-500">TELÉFONO</label>
                                <input className="w-full border border-slate-300 rounded p-2 mt-1" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                            </div>
                        </div>
                        <div>
                             <label className="text-xs font-mono font-bold text-slate-500">DIRECCIÓN</label>
                             <input className="w-full border border-slate-300 rounded p-2 mt-1" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
                        </div>
                        <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                             ...
                             <input type="number" min="0" className="w-full border border-blue-200 rounded p-2 mt-1 font-bold text-lg" value={formData.creditLimit} onChange={e => setFormData({...formData, creditLimit: e.target.value})} placeholder="0.00" />
