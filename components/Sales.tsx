    ...
    return <div className="nx-app-shell min-h-full bg-surface-950 p-4 text-slate-100 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-6xl">
            <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div><p className="text-xs font-black uppercase tracking-[0.18em] text-brand">Operación</p><h1 className="mt-1 text-3xl font-black tracking-tight text-white">Ventas</h1><p className="mt-1 text-sm text-slate-400">Buscá comprobantes, devolvé productos o anulá errores sin borrar el historial.</p></div>
                <button type="button" onClick={() => void (tab === 'history' ? loadSales() : loadRequests())} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/10 px-4 text-sm font-bold hover:bg-white/5"><RefreshCw size={16} />Actualizar</button>
            </header>
            ...
            <div className={`mb-5 grid ${canManagePending ? 'grid-cols-3' : 'grid-cols-2'} gap-2 rounded-2xl border border-white/10 bg-surface-900 p-1.5`}>
                <button type="button" onClick={() => setTab('history')} className={`rounded-xl px-4 py-3 text-sm font-bold ${tab === 'history' ? 'bg-brand text-white' : 'text-slate-400'}`}>Historial de ventas</button>
                <button type="button" onClick={() => setTab('approvals')} className={`rounded-xl px-4 py-3 text-sm font-bold ${tab === 'approvals' ? 'bg-brand text-white' : 'text-slate-400'}`}>Aprobaciones</button>
                {canManagePending && <button type="button" onClick={() => setTab('pending')} className={`rounded-xl px-4 py-3 text-sm font-bold ${tab === 'pending' ? 'bg-brand text-white' : 'text-slate-400'}`}>Pendientes</button>}
            </div>
            ...
            {tab === 'approvals' && <div className="space-y-3">
                ...
                {loading ? <div className="p-12 text-center text-slate-400"><Loader2 className="mr-2 inline animate-spin" />Cargando solicitudes…</div> : requests.length === 0 ? <div className="rounded-2xl border border-white/10 bg-surface-900 p-12 text-center text-slate-400">No hay solicitudes.</div> : requests.map((request) => <div key={request.id} className="rounded-2xl border border-white/10 bg-surface-900 p-4 sm:flex sm:items-center sm:justify-between sm:gap-4">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2 sm:mt-0"><span className={`rounded-full px-3 py-1 text-[11px] font-black ${request.status === 'PENDING_APPROVAL' ? 'bg-amber-500/10 text-amber-300' : request.status === 'COMPLETED' ? 'bg-emerald-500/10 text-emerald-300' : 'bg-slate-500/10 text-slate-300'}`}>{request.status}</span>{request.status === 'PENDING_APPROVAL' && <button type="button" disabled={workingRequestId === request.id || !approverEmail || !approverPassword} onClick={() => void approveRequest(request)} className="rounded-lg bg-brand px-3 py-2 text-xs font-black text-white disabled:opacity-40">{workingRequestId === request.id ? <Loader2 className="animate-spin" size={14} /> : 'Aprobar'}</button>}{request.status === 'APPROVED' && <button type="button" disabled={workingRequestId === request.id} onClick={() => void executeRequest(request)} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-black text-white disabled:opacity-40">{workingRequestId === request.id ? <Loader2 className="animate-spin" size={14} /> : 'Ejecutar'}</button>}</div>
                </div>)}
            </div>}
