...
export default function PendingCorrections() {
    ...
    return <div className="space-y-6">
        ...
        <section><h2 className="mb-3 flex items-center gap-2 text-lg font-black"><ReceiptText size={19} className="text-sky-400" />Reembolsos externos pendientes</h2>
            <div className="space-y-3">{refunds.length === 0 ? <p className="rounded-2xl border border-white/10 bg-surface-900 p-6 text-sm text-slate-400">No hay reembolsos externos pendientes.</p> : refunds.map((refund) => <div key={refund.id} className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-surface-900 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold text-white">{refund.sale.customerName || 'Cliente de mostrador'} · {formatMoney(refund.amount)}</p><p className="text-xs text-slate-400">{refund.method} · Factura {refund.sale.invoiceSeries ?? 'A'}-{refund.sale.invoiceNumber ?? '—'}</p></div><button type="button" disabled={working === refund.id || reference.trim().length < 4 || note.trim().length < 4} onClick={() => void completeRefund(refund.id)} className="rounded-xl bg-sky-600 px-4 py-2 text-sm font-black text-white disabled:opacity-40">{working === refund.id ? <Loader2 className="animate-spin" size={16} /> : <><CheckCircle2 className="mr-2 inline" size={16} />Marcar enviado</>}</button></div>)}</div>
        </section>
        ...
        <section><h2 className="mb-3 flex items-center gap-2 text-lg font-black"><PackageCheck size={19} className="text-amber-400" />Productos en cuarentena</h2>
            <div className="space-y-3">{inspections.length === 0 ? <p className="rounded-2xl border border-white/10 bg-surface-900 p-6 text-sm text-slate-400">No hay productos esperando inspección.</p> : inspections.map((inspection) => <div key={inspection.id} className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-surface-900 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold text-white">{inspection.product.name}</p><p className="text-xs text-slate-400">{inspection.quantity} {inspection.product.unit} · SKU {inspection.product.sku}</p></div><div className="flex gap-2"><button type="button" disabled={working === inspection.id} onClick={() => void resolveInspection(inspection.id, 'DISCARD')} className="rounded-xl border border-danger/30 px-3 py-2 text-xs font-black text-danger">Descartar</button><button type="button" disabled={working === inspection.id} onClick={() => void resolveInspection(inspection.id, 'RESTOCK')} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white">Liberar a stock</button></div></div>)}</div>
        </section>
