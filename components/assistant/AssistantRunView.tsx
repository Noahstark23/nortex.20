...
export function AssistantRunView({ run, busy, onCancel, onRecover, onReview, onInvestigate, investigationDisabled, onSaveWork }: { run: AssistantRunDTO; busy: boolean; onCancel: () => void; onRecover: () => void; onReview: (id: string) => void; onInvestigate?: (shiftId: string, reportHash?: string) => void; investigationDisabled?: boolean; onSaveWork?: () => void }) {
    ...
    return <section aria-label="Trabajo de NortexGPT" className="nx-shell-control min-w-0 space-y-3 rounded-card border p-4">
        ...
        {run.status === 'SUCCEEDED' && onSaveWork && run.result?.evidence.some(item => item.tool === 'review_weekly_cash') && <button type="button" className={assistantButtonClass} disabled={busy} onClick={onSaveWork}>Guardar revisión como trabajo</button>}
