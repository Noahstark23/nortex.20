...
export function AssistantRunView({ run, busy, onCancel, onRecover, onReview }: { run: AssistantRunDTO; busy: boolean; onCancel: () => void; onRecover: () => void; onReview: (id: string) => void }) {
    ...
    return <section aria-label="Trabajo de NortexGPT" className="nx-shell-control min-w-0 space-y-3 rounded-card border p-4">
        ...
        {run.result && <><p className="nx-shell-text whitespace-pre-wrap break-words text-sm">{run.result.text}</p>{run.result.degraded && <p className="nx-tone-warning text-sm">Parte de la información no estuvo disponible. Revisá las fuentes y los datos pendientes.</p>}
            {run.result.evidence.map(evidence => <div key={evidence.id}><AssistantOperationalEvidence evidence={evidence} /></div>)}
