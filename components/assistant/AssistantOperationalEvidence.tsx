...
import { AssistantKnowledgeCitations } from './AssistantKnowledgeSource';
...
function HelpEvidence({ data }: { data: AssistantJsonObject }) {
    const citations = Array.isArray(data.citations) ? data.citations.flatMap(value => {
        ...
        return [{ id: String(citation.id), title: String(citation.title), section: String(citation.section), version: String(citation.version), path: '', ...(typeof citation.sectionId === 'string' ? { sectionId: citation.sectionId } : {}), ...(typeof citation.contentHash === 'string' ? { contentHash: citation.contentHash } : {}) }];
    }) : [];
    return <>
        ...
        {citations.length > 0 && <AssistantKnowledgeCitations citations={citations} />}
    </>;
}
