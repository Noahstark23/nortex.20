...
export const AssistantInvoiceReview: React.FC<Props> = ({ proposal, capabilities, request, busy, operation, onSave, onConfirm, onOpenPurchases, editor, confirmationRejected = false }) => {
    ...
    const ready = !dirty && (confirmStarted || !catalog.changed) && proposal.status === 'READY' && !!proposal.preview && proposal.issues.length === 0 && cashIdentified;
    const confirm = async () => { if (!confirmStarted && (!catalog.verified || catalog.changed)) return; setConfirmStarted(true); await onConfirm(confirmation.current.key); };
    ...
    return <section aria-label={manual ? 'Revisar compra' : 'Revisar factura'} className="space-y-4">
        ...
        {!confirmStarted && canPrepare && <button type="button" className={`${assistantButtonClass} w-full`} disabled={locked || !catalog.verified || (!dirty && proposal.status !== 'DRAFT')} onClick={() => { if (catalog.verified) void onSave({ ...draft, warnings: warningsReviewed ? [] : draft.warnings }); }}>{busy ? 'Comprobando…' : 'Guardar revisión y calcular efectos'}</button>}
        {proposal.preview && !dirty && (confirmStarted || !catalog.changed) && <section aria-label="Efectos de la compra" className="nx-shell-control space-y-3 rounded-card border p-4">
