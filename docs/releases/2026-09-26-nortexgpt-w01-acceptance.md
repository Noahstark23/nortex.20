...
- MySQL 8 efímero: `npm run test:integration:required` aprobó **52 suites, 564 casos, cero omitidos**. La suite W01 pasó 5 casos: upgrade de tabla poblada y estado parcial con `db push` repetido, concurrencia, hash exacto, replay del comprobante, identidad/revocación/fuente y cero escrituras de caja/IA.
...
La prueba de upgrade se ejecutó sólo sobre la base descartable. Antes de cualquier promoción con schema siguen siendo necesarios un respaldo off-site reciente y un restore drill vigente de la base real, CI y staging del mismo SHA y autorización separada de producción. El trabajo W01 aún requiere comprobar reinicio real, navegación con carrito y evaluación humana/modelo/piloto.
